...
class User(db.Model):
    ...
    alfred_files = db.relationship("AlfredFile", backref="user", lazy=True)
...
class AgentStatus(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    platform = db.Column(db.String(50), nullable=False, unique=True)  # 'telegram', 'discord', 'whatsapp'
...
class Message(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=True)  # null para mensagens do bot
